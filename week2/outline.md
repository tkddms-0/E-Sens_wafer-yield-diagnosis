...
ex. ADR 1: 데이터 분할은 lotName 기준 그룹 분할로 한다
- 날짜: 2026-09-25
- 상황: 같은 lot의 웨이퍼는 결함 패턴이 유사해, 무작위 분할 시 train/test에 같은 lot이 섞여 성능이 과대평가됨
- 결정: GroupShuffleSplit(groups=lotName) 사용
- 버린 대안: 무작위 분할 (누수), waferIndex 기준 분할 (lot 의존성 분석에 필요한 변수라 부적합)
- 결과: 베이스라인 balanced accuracy 무작위 분할 대비 __%p 하락 — 이 값이 정직한 기준선
